...
(() => {
  ...
  const rawTrackKey = (params.get("track") || "australia");
  let trackKey = rawTrackKey.toLowerCase();
  ...
  async function loadTrackSVG() {
    ...
    // Etapa 1: resolver pista via TrackKeyMap/TrackRenderer (corrige SVG cortado e carros invisíveis)
    if (window.TrackKeyMap && window.TrackRenderer) {
      try {
        const res = await window.TrackRenderer.renderTrack({
          containerEl: trackContainer,
          rawTrackKey: trackKey,
          basePath: "assets/tracks/",
          samples: 1400
        });
        trackKey = res.resolvedKey;
        svgRoot = res.svgRoot;
        mainPath = res.mainPath;
        carsLayer = res.carsLayer;
        pathPoints = res.points;
        curvature = buildCurvature(pathPoints);
        window.pathPoints = pathPoints;
        safeSetText(elSessionStatus, "Treino Livre");
        return; // já carregou a pista com sucesso
      } catch (e) {
        console.warn("[practice] TrackRenderer falhou, caindo para loader interno:", e);
      }
    }
